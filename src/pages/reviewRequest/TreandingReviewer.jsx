...
import { FilterAllTrendingReviewers, FilterAllTrendingUsers, FollowUser, UnFollow, userDetails } from '../../services/api_services'
...
function TreandingReviewer() {
    ...
    const SuggestedUser = async () => {
        await FilterAllTrendingReviewers(userBody).then(res => {
        ...
        <div className='suggest_users'>
            <Card>
                <div className="suggest_user_heading">
                    <p>Trending Reviewer</p>
                </div>
                {initialPosts.map((item, index) => {
                    return (
                        <div key={item.USERID}>
                            {item.USERID != userDetails.userID && <div >
...
export default TreandingReviewer
