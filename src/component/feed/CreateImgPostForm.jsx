...
function CreateImgPostForm({ handleCancel }) {
   ...
   const taghandleChange = (tags) => {
    ...
    const submithandler = async () => {
        await CreateRegularPost(postBody).then(res => {
            ...
            message.error(err.responce.data.extras.Status);
    ...
    useEffect(() => {
        ...
        // uploadImg != "" && mediaType==2 && uploadGifImage() 
    }, [uploadImg])
    ...
    const ListAllspecialist = async () => {
        await filterAllspecialisation(speceliseBody).then(res => {
            ...
            if (res.data.success) {
                ...
                // console.log("dept", spacialisationList);
            }
        }).catch(err => {
            message.error(err.response.data.extras.msg);
    ...
    const commenthandle = (e) => {
        ...
        // console.log(`comment = ${e.target.checked}`);
    };
    const downloadhandle = (e) => {
        setDownload(e.target.checked)
        // console.log(`download = ${e.target.checked}`);
    };
    const ownerhandle = (e) => {
        setOwner(e.target.checked)
        // console.log(`owner = ${e.target.checked}`);
    };
    return (
        <div>
...
export default CreateImgPostForm
